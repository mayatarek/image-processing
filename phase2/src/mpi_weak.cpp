...
int main(int argc, char** argv) {
    ...
    // for weak scaling, change size of image proportional to number of processes
        int base_size = 4000; // size for 1 process
        // base_size is the hieght, base_size * nprocs is the width
        resize(img, img, Size(base_size, base_size * nprocs));  
...
// for strong scaling analysis, save times to txt file so automate calcultions 
    if (rank == 0) {
        ...
        // save to file
        ofstream outfile("mpi_times_weak.txt", ios::app); // append mode
        if (outfile.is_open()) {
        outfile << nprocs << " " << elapsed << " " << rows << "x" << cols << "\n";
            outfile.close();
        }
    }
